...
## 유형별 시그널 층화 (2R 수집 후 신설 — `labeling/manipulation_typology.md` 연계)

실사례 38건 분석 결과, **조작 유형별로 유효 시그널 축이 다르다**:
- **T1 소매홍보형** → P2(촉매 케이던스), FIN→PROMO, sentiment 2차
- **T2 은밀매집형** → **P1(BCI)·P4(APS)** — 1차 시그널의 주 타깃. SG·영풍제지가 실증 근거
- **T3 MM/봇형** → ⚠️ **현 카탈로그에 대응 시그널 없음.** 후보(미설계): 특정 계정 거래비중 극단(D코인 89% 사례), 호가 취소율(spoof), self-trade 근사(wash), 거래량 대비 고유 참여자 수 괴리. crypto fold 필수 — **v0.3 시그널 설계 과제**
- **T5 착시·군중형** → 촉매 구조 자체(권리락·품절) + S1/S3 동학
- LR 산출 시 유형 fold 분리 권장(유형별 기저율·시그널 유효성이 다름 → pooling 왜곡)
